...
public class AiRoutingService: IAiRoutingService
{
    ...
    public async Task<StructuredAddress> ExtractAddressAsync (string rawAddress)
    {
        ...
        var response = await _httpClient.PostAsJsonAsync("api/extract-address", requestBody,writeOptions); //POST and automatically serializes 'requestBody' into JSON format for the request body.
